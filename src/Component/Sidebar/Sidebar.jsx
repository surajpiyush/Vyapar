                     ...
                     {item?.rightSideIcon == "plus" ? (
                        <PlusIconThin
                           onClick={(e) => {
                              ...
                              if (activeNav == "/items")
                                 return setShowAddItemsForm(true);
                           }}
                        />
...
const SidebarItems = [
   ...
   },
   {
      ...
      to: "/",
      childLinks: [
         { name: "Generate Barcode", to: "/" },
         { name: "Import Items", to: "/" },
         { name: "Bulk Update Items", to: "/" },
         { name: "Import From Tally", to: "/" },
...
function findNameByPathnameRecurrsive(arr, pathname) {
   for (let i = 0; i < arr.length; i++) {
      const obj = arr[i];
      if (obj.childLinks) {
         const foundChild = findNameByPathnameRecurrsive(
            obj.childLinks,
            pathname
         );
         if (foundChild) {
